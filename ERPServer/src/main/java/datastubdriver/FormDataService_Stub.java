 ...
 */
public class FormDataService_Stub {
    ...
    public ArrayList<String> getAllIDByType(BillType type) throws RemoteException {
        ...
        switch (type) {
            case CASH:
                IDs.add(Integer.valueOf(billVO3.getID()).toString());
                break;
            case GIFT:
                for (int i = 0; i < inventoryBillPOS.size(); i++){
                    if (inventoryBillPOS.get(i).getType().equals(BillType.GIFT))
                        IDs.add(Integer.valueOf(inventoryBillPOS.get(i).getID()).toString());
                }
                break;
            case LOSS:
                for (int i = 0; i < inventoryBillPOS.size(); i++){
            ...
            case ALARM:
                ...
                break;
            case SALES:
                IDs.add(Integer.valueOf(s1.getID()).toString());
                break;
            case PAYMENT:
                IDs.add(Integer.valueOf(billVO2.getID()).toString());
                break;
            case RECEIPT:
                IDs.add(Integer.valueOf(billVO1.getID()).toString());
            case OVERFLOW:
                for (int i = 0; i < inventoryBillPOS.size(); i++){
